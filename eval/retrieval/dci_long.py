...
def run_dci_long_openclip(model, processor, data_path):
    ...
    with torch.no_grad():
        ...
        for images, captions in tqdm(dataloader, desc="Processing batches"):
            texts_encoded = longclip.tokenize(captions, truncate=True).to(device)
            text_features = model.encode_text(texts_encoded)

            images_encoded = images.to(device)
            images_encoded = images_encoded.to(device)
            ### TODO: debug
            # print("images_encoded: ", images_encoded.shape)
...
def main():
    ...
    if language == "en":
        data_path = 'data/dci'
    run_dci_long_openclip(model, preprocess, data_path)
